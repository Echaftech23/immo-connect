...
class Reply implements Replyinterface 
{
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM messages WHERE message_id = ?");
        $stmt->execute(array('message_id' => $message_id));// fi chek tkhdm
        $messages = $stmt->fetchAll(PDO::FETCH_OBJ);
        if ($messages){
            $message = new Message($messages->id, $messages->content, $messages->datePublication, $messages->status, $messages-> message_id);
            return $message;
        }
        return false;
    }
